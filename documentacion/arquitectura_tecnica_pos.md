...
- **Login por username o email (observación de sesión, punto 1 de la ronda siguiente) — un solo mecanismo, dos identificadores.** Una cuenta tiene `username` (obligatorio, único a nivel sistema, no por tenant — no repite el bug de colisión de pharma_core) y `email` (opcional). Ambos identifican a la MISMA cuenta y validan contra la MISMA contraseña real — no son dos formas de autenticación, es una sola: resolver la cuenta por uno u otro y luego chequear password, igual de fuerte en ambos casos. Implementado en `tenants.serializers.IdentifierTokenObtainPairSerializer` (reemplaza el `TokenObtainPairSerializer` estándar de SimpleJWT) / `tenants.services.authenticate_by_identifier`, único endpoint `POST /api/v1/auth/token/`.

  Corrección explícita: una ronda anterior de esta sesión interpretó "identificador alterno" como un SEGUNDO mecanismo de autenticación independiente — `username` + fecha de nacimiento en vez de contraseña, sin validar password en absoluto. Eso fue un malentendido real, no una variante válida, y se removió por completo (endpoint, serializer, servicio, y todos sus tests) en vez de dejarlo desactivado. `UserProfile.date_of_birth` se conserva únicamente como dato de perfil administrativo — nunca se usa para autenticar a nadie.
